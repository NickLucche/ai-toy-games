...
class CSPNode:
    ...
    def remove_value_from_domain(self, value: int):
        self.domain = np.asarray([vx for vx in self.domain if vx != value], dtype=np.int32)
...
class ConstraintGraph:
    ...
    def __init__(self, game: CSP, generate_arcs=True) -> None:
...
def backtrack_search(
        game: CSP, csp: ConstraintGraph, get_variable: Callable[[CSP],
                                                                CSPNode],
        get_domain_value: Callable[[CSP, ConstraintGraph, CSPNode],
    ...
    var = get_variable(game, csp)
    # 2. Select value from respective domain wrt some ordering
    for value in get_domain_value(game, csp, var):
        # assign variable
        ...
        if consistent:
            ...
            if inferences is not None:
                ...
                res = backtrack_search(game, csp, get_variable,
                                       get_domain_value, inference)
                if res is not None:
                    return res
...
def mrv_heuristics(game: CSP, csp: ConstraintGraph) -> CSPNode:
    ...
    """
    dom_lengths = np.asarray([len(node.domain) for node in csp.nodes], dtype=np.int32)
...
def forward_checking(game: CSP, csp: ConstraintGraph, ass: Tuple[CSPNode,
    ...
    for y in csp.generate_neighbors(x, exclude_neighbors=assigned_variables):
        ...
        if len(new_y_domain) == 0:
            print(f"Forward checking detected an arc inconsistency between node {x.var_id} and {y.var_id}")
            return None
